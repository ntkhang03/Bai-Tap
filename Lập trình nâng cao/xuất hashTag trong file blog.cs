...
namespace CSharp_Shell {
  ...
  public static class Program {
	  public static void Main() {
	  	// đọc file blog.txt và xuất ra tất cả hashTag vào file Hashtag.txt
	    StreamReader baiBao = new StreamReader("blog.txt");
	    StreamWriter writeHashtag = new StreamWriter("Hashtag.txt");
	    string line;
	    string outputHashtag = "";
	    
	    // dùng foreach + IndexOf
      while ((line = baiBao.ReadLine()) != null) {
        string[] splitLine = line.Split(); // tách chuỗi bằng ký tự khoảng cách
		    foreach (string hashTag in splitLine) // foreach duyệt từng phần tử của mảng, tài liệu tham khảo: https://howkteam.vn/course/khoa-hoc-lap-trinh-c-can-ban/vong-lap-foreach-trong-lap-trinh-c-co-ban-1193
		      if (hashTag.IndexOf('#') == 0) outputHashtag += $"{hashTag}\n"; // nếu có ký tự # đứng ở vị trí đầu tiên => là hashTag => cộng thêm vào biến outputHashtag
	    }
	    
	    writeHashtag.Write(outputHashtag); // viết nội dung trong biến outputHashtag vào file Hashtag.txt
	    writeHashtag.Close(); 
	    baiBao.Close();
	    
	    Console.WriteLine(outputHashtag); // xuất nội dung biến outputHashtag
	    Console.WriteLine(outputHashtag); // xuất nội dung biến outputHashtag
	  }
  }
}
